...
\dot{\lambda} = 0 
\eeq
と書き換えられ、$\lambda = \lambda (r)$と時間に依らない形に書ける。\eqref{eq:10009}\eqref{eq:t10010}から、
\beq
\lap + \nup = 0
\eeq
...
\epikcf \int^r_0  T^0_0 r^2dr &= \int^r_0 (r \lambda^\prime - 1)e^{-\lambda} + 1 dr\\ 
                            &= [-re^{-\lambda}]^r_0 + r = -r e^{-\lambda} + r = r(1 - e^{-\lambda})\\ 
\lambda &= - ln\left(1 - \epikcf \frac{1}{r} \int^r_0  T^0_0 r^2 dr\right)\label{eq:lambda}
\end{align}
$T^0_0$の符号は一般的には決まった符号ではないが、ここでは
\beq
T^0_0 = g^{0i}T_{0i} = g^{00} T_{00} = e^{-\nu} T_{00} \geq 0
\eeq
$\lambda$の式\eqref{eq:lambda}を見ると$1 - \epikcf \frac{1}{r} \int^r_0  T^0_0 r^2 dr \leq 1$より、
\beq
\lambda \geq 0
\eeq
\beq
...
\beq
\lambda = - ln\left(1 - \epikcf \frac{1}{r} \int^a_0  T^0_0 r^2 dr\right)
\eeq
一方で、\eqref{eq:ds}から
\beq
...
動径方向は\eqref{eq:sr}において、$r(r - r_g) = r^{\prime 2}$つまり$r\sqrt{\rgr} \sim r - \frac{r_g}{2} = r^\prime$と変数変換する。
また、$E = mc^2 + E^\prime$と静止エネルギーを分離する。
\beq
\frac{r^2}{r^{\prime 2}} = \frac{(r^{\prime} + \frac{r_g}{2})^2}{r^{\prime 2}} = 1 + \frac{r_g}{r^\prime} + \frac{r_g^2}{2r^{\prime 2}}
\eeq
を準備して、
\begin{align}
    S_r &= \int \left[(1 - \frac{r_g}{r})^{-2} \frac{E^2}{c^2} - (\rgr)^{-1}(\frac{M^2}{r^2} + m^2c^2)
 \right]^{\half}dr\\
        &= \int \left[(1 - \frac{r_g}{r})^{-2} \frac{(mc^2 + E^\prime)^2}{c^2} - (\rgr)^{-1}(\frac{M^2}{r^2} + m^2c^2)
 \right]^{\half}dr\\
        &= \int \left[(\frac{r^2}{r^{\prime 2}})^2 \frac{(mc^2 + E^\prime)^2}{c^2} - \frac{r^2}{r^{\prime 2}}(\frac{M^2}{r^2} + m^2c^2)
 \right]^{\half}dr^\prime\\
        &= \int \left[(\frac{r^2}{r^{\prime 2}})^2 \frac{m^2c^4 + 2mc^2E^\prime + E^{\prime 2}}{c^2} - \frac{1}{r^{\prime 2}}M^2 + \frac{r^2}{r^{\prime 2}}m^2c^2
 \right]^{\half}dr^\prime\\
        &= \int \left[(1 + 2\frac{r_g}{r^\prime} + 2\frac{r_g^2}{r^{\prime 2}}) (m^2c^2 + 2mE^\prime + \frac{E^{\prime 2}}{c^2}) - \frac{1}{r^{\prime 2}}M^2 - (1 + \frac{r_g}{r^\prime} + \frac{r_g^2}{2r^{\prime 2}})
m^2c^2
 \right]^{\half}dr^\prime\\
        &= \int \left[(1 + 2\frac{r_g}{r^\prime} + 2\frac{r_g^2}{r^{\prime 2}}) (m^2c^2 + 2mE^\prime + \frac{E^{\prime 2}}{c^2}) - \frac{1}{r^{\prime 2}}M^2 - (1 + \frac{r_g}{r^\prime} + \frac{r_g^2}{2r^{\prime 2}})
m^2c^2
 \right]^{\half}dr^\prime\\
        &= \int \left[
            (2E^\prime m + \frac{E^\prime}{c^2}) + \frac{1}{r}(2m^2m^\prime k + 4 E^\prime m r_g) - \revrt (M^2 - \frac{3m^2c^2r_g^2}{2})
        \right]^{\half}dr\\
\end{align}
最後の式で、$r^\prime$を$r$と書き換えた。
このうち第三項目の$\revrt$が近日点の系統的な永年移動をもたらす。\\
%todo もっと深く
軌道について考える。軌道は、\eqref{eq:kidou}によって決まる。
\beq
\phi + \pder[S_r]{M} = const
\eeq
惑星が軌道を一回りする時間の間の角度$\phi$の変化は、
\beq
\Delta \phi = - \pder[]{M} \Delta S_r
\eeq
$\Delta S_r$を展開する。
\beq
\Delta S_r = \Delta S_0 - \frac{3m^2c^2r_g^2}{2}\frac{1}{2M}\pder[S_0]{M} = \Delta S_0 - \frac{3m^2c^2r_g^2}{4M}\pder[S_0]{M}
\eeq
ここで、$\Delta S_0$は近日点移動のない閉じた楕円を描く運動に対応するので、
\beq
\Delta \phi = - \pder[]{M} \Delta S_0 = 2\pi
\eeq
よって
\beq
\Delta \phi = 2\pi + \frac{3\pi m^2c^2r_g^2}{2M^2} = 2\pi + \frac{6\pi k^2 m^2 m^{\prime 2}}{c^2M^2}
\eeq
第二項が近日点移動で、
\beq
\delta \phi = \frac{6\pi k^2 m^2 m^{\prime 2}}{c^2M^2}
\eeq
とかける。公式
\beq
\frac{M}{km^\prime m^2} = a(1 - e^2)
\eeq
を使えば、
\beq
\delta \phi = \frac{6\pi km^{\prime}}{c^2 a(1 - e^2)}
\eeq
%todo 公式
\subsection{光線の径路}
アイコナール方程式が光線の径路を決める。
